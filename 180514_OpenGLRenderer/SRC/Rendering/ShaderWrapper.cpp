...
*	@return void.
*/
void ShaderWrapper::LoadShader(const char * a_filePath, unsigned int a_shaderType)
{
...
void ShaderWrapper::SetTexture(const char * a_name, TextureWrapper * a_tex)
{
...
void ShaderWrapper::SetMaterial(const char * a_name, Material a_mat)
{
	...
	SetBool((nameStr + ".useDiffuseMap").c_str(), (a_mat.diffuseMap ? true : false));
	SetBool((nameStr + ".useSpecularMap").c_str(), (a_mat.specularMap ? true : false));
}
