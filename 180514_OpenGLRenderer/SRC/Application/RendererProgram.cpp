...
#pragma region Models/VertFormats/Meshes
		// Models
		nanosuitModel = new Model("./models/nanosuit/nanosuit.obj");
		floorModel = new Model("./models/floor/Sci-Fi-Floor-1-BLEND.obj"); floorModel->GetTransform()->SetScale(glm::vec3(10.f, 10.f, 10.f));
...
#pragma endregion
	...
	void RendererProgram::FixedUpdate(float a_dt)
	{
		...
		while (m_accumulatedTime >= m_fixedTimeStep) {
			...
			nanosuitModel->GetTransform()->SetRotation(glm::vec3(0.f, glm::radians(glfwGetTime()) * rotSpeed, 0.f));
...
#pragma endregion Gizmo creation
	...
	void RendererProgram::Render()
	{
		...
		// Models
		nanosuitModel->Draw(mainCamera, sceneLights, globalAmbient, ambientProgram, directionalProgram, pointProgram, flashLight);
		floorModel->Draw(mainCamera, sceneLights, globalAmbient, ambientProgram, directionalProgram, pointProgram, flashLight);
